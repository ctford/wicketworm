#!/usr/bin/env python3
"""
Train XGBoost model for Test match W/D/L prediction - SCORECARD ONLY VERSION
This version uses only visible match state (no team ratings or home advantage)
"""
...
def extract_scorecard_features(states: List[GameState]) -> pd.DataFrame:
    """
    Extract 5 scorecard-only features (no team context like ratings or home advantage)
...
def export_model(model, label_encoder, feature_names: List[str], output_dir: Path) -> None:
    """Export XGBoost model for server-side inference"""
    # Save model and label encoder using pickle
    model_path = output_dir / "model_scorecard_only.pkl"
    ...
    metadata = {
        ...
        "model_type": "xgboost_scorecard_only"
    }

    metadata_path = output_dir / "model_metadata_scorecard_only.json"
...
def main():
    ...
    print("WicketWorm Model Training (XGBoost - Scorecard Only)")
    print("=" * 50)
    ...
    # Extract features
    print("\n2. Extracting scorecard-only features (no team ratings/home advantage)...")
    df = extract_scorecard_features(states)
